...
@StepScope
@Slf4j
public class SmaregiProductItemReader extends AbstractSmaregiItemReader {

    private final SmaregiApiProperties properties;

    public SmaregiProductItemReader(RestTemplate restTemplate,
    ...
    protected URI getApiUrl(int page) {
        String baseUrl = properties.getBaseUrl() + "/" + properties.getContractId() + "/pos/products";
        try {
        String urlString = String.format("%s?limit=1000&page=%d", baseUrl, page);
        log.info("Request URL for Products: {}", urlString);
