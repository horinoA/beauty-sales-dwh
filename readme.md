...
## Summary
顧客データの統合と売上分析を目的としたDWH基盤を、設計〜実装〜可視化まで一貫して構築したプロジェクトです。
* POS APIからのデータ取得〜ETL処理（Spring Batch）
* Raw / DWH分離による分析基盤設計
* 顧客名寄せアルゴリズムの設計・実装（スコアリング＋ブロッキング）
* ダッシュボードによる経営指標の可視化
...
* Levenshtein距離: PostgreSQLのfuzzystrmatch拡張のlevenshtein関数を活用
